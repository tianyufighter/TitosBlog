...
class _TodoCompletePageState extends State<TodoCompletePage> {
  ...
  Future getNoTodoList() async {
    _page = 1;
    List<Todo> result = await apiClient.getTodoByPage(pageNum: _page, username: Constant.username, type: todoType, status: 1);
    if(result.length > 0) {
  ...
  Future updateTodoState(int _id, int index) async {
    ...
    _showLoading(context);
    CommonResult result = await apiClient.updateStatusById(id: _id, status: 0);
    _dismissLoading(context);
    if(result.code == 200) {
